...
login.post('/diner/login', async (req,res)=>{
    ...
    const check = await db.collection("Diner")
    ...
    if(check){
        ...
        const accToken = jwt.sign({email: dinerEmail}, process.env.ACCESS_TOKEN_SECRET, {expiresIn:'1h'})
        const refToken = jwt.sign({email: dinerEmail}, process.env.REFRESH_TOKEN_SECRET)
        res.json({
